...
export default function App() {
  ...
  return (
    <div className={`min-h-screen flex flex-col relative transition-all duration-700 ease-in-out`}>
      {/* Background Glow with enhanced transition */}
      <div className={`fixed top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] ${theme === 'dark' ? 'bg-amber-400/5' : 'bg-amber-500/10'} blur-[120px] rounded-full -z-10 pointer-events-none transition-all duration-1000 ease-in-out`}></div>
      ...
      <header className="w-full max-w-7xl mx-auto px-6 pt-10 pb-6 flex justify-between items-center z-20">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className={`text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'} leading-none bn-serif tracking-tight transition-colors duration-700`}>
              পবিত্র <span className="text-divine-gold">বানী</span>
            </h1>
            <p className="text-[14px] text-amber-600 font-bold mt-2 bn-serif transition-colors duration-700">জ্ঞানের আলো, আত্মার শান্তি</p>
          </div>
        </div>
          ...
          <div className="space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-8 md:pb-0">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="relative group">
                ...
                <div className="relative">
                  <input 
                    ...
                    className={`w-full ${theme === 'dark' ? 'bg-slate-900/40' : 'bg-white/80'} backdrop-blur-3xl border border-white/10 pl-10 pr-40 md:pr-64 py-8 rounded-[2.5rem] text-xl md:text-2xl outline-none focus:ring-2 ring-amber-500/50 transition-all duration-700 ${theme === 'dark' ? 'placeholder-slate-600 text-white' : 'placeholder-slate-400 text-slate-900'} font-bold shadow-3xl bn-serif`}
                  />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1 mt-2">
                    {filteredAutoSuggestions.map((suggestion, idx) => (
                      <button
                        ...
                        className={`text-left px-6 py-4 rounded-xl transition-all duration-700 ${theme === 'dark' ? 'hover:bg-white/5 text-slate-200' : 'hover:bg-black/5 text-slate-800'} font-bold bn-serif flex items-center gap-4 group`}
                      >
            ...
            {state === AppState.SEARCHING && (
              <div className="flex flex-col items-center py-24 gap-8 animate-pulse">
                ...
                <div className="text-center space-y-3">
                  <h3 className={`${theme === 'dark' ? 'text-white' : 'text-slate-800'} text-3xl font-black bn-serif transition-colors duration-700`}>ঐশ্বরিক বাইবেলীয় অন্তর্দৃষ্টি খোঁজা হচ্ছে</h3>
            ...
            {state === AppState.ERROR && (
              <div className="max-w-2xl mx-auto divine-glass p-12 md:p-16 rounded-[4rem] text-center border-rose-500/20 shadow-2xl">
                ...
                <h3 className={`${theme === 'dark' ? 'text-white' : 'text-slate-900'} text-3xl font-black mb-4 bn-serif transition-colors duration-700`}>তথ্য বিভ্রাট</h3>
            ...
            {currentVerse && state === AppState.IDLE && (
              <div className="max-w-5xl mx-auto space-y-16 pb-24 animate-in slide-in-from-bottom-12 duration-1000">
                <div className="relative group">
                  ...
                  <div className={`relative ${theme === 'dark' ? 'bg-slate-900/50' : 'bg-white/70'} backdrop-blur-3xl p-12 md:p-20 rounded-[4rem] border border-white/5 overflow-hidden text-center shadow-3xl transition-all duration-700`}>
                    ...
                    <div className="mb-12 relative">
                      <h2 className={`${mainTextSizeClass} font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'} leading-relaxed bn-serif px-8 drop-shadow-sm max-w-4xl mx-auto relative z-10 text-justify transition-colors duration-700`}>
                        {currentVerse.text}
                      </h2>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <div className="divine-card p-10 md:p-12 rounded-[3rem] flex flex-col justify-between hover:border-amber-500/20 transition-all duration-700 group">
                  ...
                  <div className="divine-card p-10 md:p-12 rounded-[3rem] flex flex-col justify-between hover:border-blue-500/20 transition-all duration-700 group">
                    <div className="space-y-6">
                  ...
                  <div className="divine-card p-10 md:p-12 rounded-[3rem] flex flex-col justify-between hover:border-emerald-500/20 transition-all duration-700 group">
                    <div className="space-y-6">
                ...
                <div className="max-w-4xl mx-auto divine-glass p-12 rounded-[4rem] space-y-8 border-amber-500/20 shadow-3xl animate-in slide-in-from-bottom-8 duration-1000 text-center transition-all duration-700">
                  ...
                  <p className={`${explanationSizeClass} ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'} leading-relaxed bn-serif font-medium text-justify relative z-10 px-4 transition-colors duration-700`}>
                    {currentVerse.prayer}
                  </p>
                ...
                <div className="flex flex-wrap justify-center gap-3 pt-6">
                   {currentVerse.keyThemes.map((themeStr, i) => (
                     <span key={i} className="px-6 py-3 rounded-full divine-glass text-xs font-black text-amber-600/70 uppercase tracking-widest border border-amber-500/10 hover:border-amber-500/40 transition-all duration-700 cursor-default shadow-sm">{themeStr}</span>
                   ))}
                </div>

                <div className="flex flex-wrap justify-center gap-6 pt-10">
                  <button onClick={toggleSave} className={`flex items-center gap-4 px-12 py-6 rounded-[2.5rem] divine-glass transition-all duration-700 border-2 ${isCurrentVerseSaved ? 'text-amber-600 bg-amber-500/10 border-amber-500/40 shadow-[0_0_40px_rgba(251,191,36,0.2)]' : 'text-slate-500 border-white/5 hover:border-amber-500/20 hover:text-slate-200'} active:scale-95`}>
                  ...
                  <button onClick={handleShare} className={`flex items-center gap-4 px-12 py-6 rounded-[2.5rem] divine-glass transition-all duration-700 border-2 text-slate-500 border-white/5 hover:border-amber-500/20 hover:text-slate-200 active:scale-95 relative group`}>
                    {showCopyFeedback && (
          ...
          <div className="animate-in fade-in slide-in-from-right-12 duration-600 space-y-12 py-10 pb-12 md:pb-0">
            <div className="flex flex-col items-center text-center gap-6 mb-12 animate-in fade-in slide-in-from-top-10 duration-1000">
               <div className="flex items-center gap-8">
                 <div className="h-px w-12 md:w-24 bg-gradient-to-r from-transparent via-amber-500/40 to-transparent transition-all duration-700"></div>
                 <i className="fa-solid fa-bookmark text-amber-500/30 text-2xl"></i>
                 <h2 className={`text-4xl md:text-5xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'} bn-serif leading-none tracking-tight transition-colors duration-700`}>আপনার <span className="text-divine-gold">সংগ্রহ</span></h2>
                 <i className="fa-solid fa-bookmark text-amber-500/30 text-2xl"></i>
                 <div className="h-px w-12 md:w-24 bg-gradient-to-l from-transparent via-amber-500/40 to-transparent transition-all duration-700"></div>
               </div>
            ...
            {savedVerses.length > 0 && (
              <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row gap-6 items-center justify-between divine-glass p-8 rounded-[3rem] border-white/5 shadow-2xl transition-all duration-700">
                   <div className="w-full md:w-2/3 space-y-4">
                      <div className="flex justify-between items-center pr-4">
                        ...
                        <button 
                          ...
                          className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-amber-500 hover:text-amber-400 transition-colors py-1 px-3 divine-glass rounded-lg border-white/5"
                        >
                      ...
                      <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide no-scrollbar">
                        <button 
                          ...
                          className={`whitespace-nowrap px-6 py-2 rounded-full transition-all duration-700 text-[11px] font-black uppercase tracking-widest border-2 ${!filterTag ? 'bg-amber-500 text-white border-amber-500 shadow-lg' : 'bg-white/5 text-slate-500 border-white/5 hover:border-amber-500/20'}`}
                        >
                          সব ট্যাগ
                        </button>
                        {uniqueTags.map(tag => (
                          <button 
                            ...
                            className={`whitespace-nowrap px-6 py-2 rounded-full transition-all duration-700 text-[11px] font-black uppercase tracking-widest border-2 ${filterTag === tag ? 'bg-amber-500 text-white border-amber-500 shadow-lg' : 'bg-white/5 text-slate-500 border-white/5 hover:border-amber-500/20'}`}
                          >
                            {tag}
                          </button>
                        ))}
                   ...
                   <div className="w-full md:w-1/3 space-y-4">
                      ...
                      <div className="relative group/theme-select">
                        <select 
                          ...
                          className={`w-full p-4 rounded-2xl border-2 transition-all duration-700 appearance-none cursor-pointer outline-none focus:border-amber-500/50 shadow-inner ${theme === 'dark' ? 'bg-white/5 border-white/5 text-slate-200' : 'bg-black/5 border-black/5 text-slate-800'} bn-serif text-sm font-bold`}
                        >
            ...
            {filteredVerses.length === 0 ? (
              <div className="divine-glass p-20 md:p-32 text-center rounded-[5rem] opacity-50 shadow-2xl max-w-4xl mx-auto border-dashed border-2 border-white/10 transition-all duration-700">
                ...
                <p className={`text-2xl bn-serif font-medium ${theme === 'dark' ? 'text-white' : 'text-slate-900'} mb-10 transition-colors duration-700`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredVerses.map(v => (
                  <div key={v.id} className="divine-card p-10 rounded-[3.5rem] group flex flex-col justify-between cursor-pointer transition-all duration-700 hover:scale-[1.03] hover:shadow-3xl hover:border-amber-500/20" onClick={() => { setCurrentVerse(v); setActiveView('SEARCH'); }}>
                    <div className="space-y-8">
                      ...
                      <p className={`${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'} bn-serif text-lg leading-relaxed line-clamp-4 font-medium text-justify transition-colors duration-700`}>{v.text}</p>
                    </div>

                    <div className="mt-10 space-y-4">
                      <div className="flex flex-wrap gap-2">
                         {v.keyThemes.slice(0, 3).map((themeStr, i) => (
                           <span key={i} className="text-[10px] font-black uppercase tracking-widest text-blue-500/60 bn-serif transition-colors duration-700">{themeStr}</span>
                         ))}
                      </div>

                      <div className="flex flex-wrap gap-2.5">
                        {(v.tags || []).map(tag => (
                          <span key={tag} className="flex items-center gap-2 px-4 py-2 bg-amber-500/5 border border-amber-500/10 rounded-full text-xs font-black text-amber-600 uppercase tracking-widest group/tag hover:border-amber-500/30 transition-all duration-700 shadow-sm">
                            {tag}
                            <button 
                        ...
                        <div className="mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {tagSuggestions.map(suggestion => (
                                <button 
                                  ...
                                  className="px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full text-[10px] font-black text-amber-500 uppercase tracking-widest hover:bg-amber-500 hover:text-white transition-all duration-700"
                                >
                                  {suggestion}
                                </button>
                              ))}
                          ...
                          <div className="flex gap-2">
                            <input 
                              ...
                              className={`flex-1 bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-xs outline-none focus:ring-1 ring-amber-500/50 duration-700 ${theme === 'dark' ? 'text-white' : 'text-slate-900'} bn-serif transition-all`}
                            />
                            <button 
        ...
        {activeView === 'SETTINGS' && (
          <div className="max-w-4xl mx-auto py-12 space-y-16 animate-in fade-in zoom-in-95 pb-20 md:pb-16">
             <div className="flex items-center gap-8 px-4">
               <div className={`w-20 h-20 ${theme === 'dark' ? 'bg-white/5' : 'bg-black/5'} rounded-[2rem] flex items-center justify-center text-slate-500 shadow-inner border border-white/5 transition-all duration-700`}>
                 <i className="fa-solid fa-sliders text-3xl"></i>
               </div>
               <div className="space-y-2">
                 <h2 className={`text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'} bn-serif transition-colors duration-700`}>অ্যাপ সেটিংস</h2>
             ...
             <div className="space-y-10">
                <div className="divine-glass p-10 md:p-14 rounded-[4rem] space-y-12 shadow-3xl transition-all duration-700">
                    ...
                    <div className={`pt-12 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} space-y-8 transition-colors duration-700`}>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                          <button 
                            ...
                            className={`p-10 rounded-[2.5rem] border-2 transition-all duration-700 flex flex-col items-center gap-4 ${theme === 'dark' ? 'bg-amber-500/10 border-amber-500/50 text-amber-500 shadow-[0_0_50px_rgba(251,191,36,0.15)]' : 'bg-black/5 border-transparent text-slate-500 hover:bg-black/10'}`}
                          >
                          ...
                          <button 
                            ...
                            className={`p-10 rounded-[2.5rem] border-2 transition-all duration-700 flex flex-col items-center gap-4 ${theme === 'light' ? 'bg-amber-500/10 border-amber-500/50 text-amber-600 shadow-[0_0_50px_rgba(251,191,36,0.15)]' : 'bg-white/5 border-transparent text-slate-400 hover:bg-white/10'}`}
                          >
                    ...
                    <div className={`pt-12 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} space-y-8 transition-colors duration-700`}>
                      ...
                      <div className="relative group/select">
                        <select 
                          ...
                          className={`w-full p-6 rounded-2xl border-2 transition-all duration-700 appearance-none cursor-pointer outline-none focus:border-amber-500/50 shadow-inner ${theme === 'dark' ? 'bg-white/5 border-white/5 text-slate-200' : 'bg-black/5 border-black/5 text-slate-800'} bn-serif text-lg`}
                    ...
                    <div className={`pt-12 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} space-y-8 transition-colors duration-700`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {['Kore', 'Zephyr', 'Charon'].map(v => (
                            <button key={v} onClick={() => handleVoiceChange(v)} className={`p-6 rounded-[2rem] border-2 transition-all duration-700 flex flex-col items-center gap-4 ${selectedVoice === v ? 'bg-amber-500/10 border-amber-500/50 text-amber-500 shadow-[0_0_50px_rgba(251,191,36,0.15)]' : 'bg-white/5 border-transparent text-slate-500 hover:bg-white/10'}`}>
                    ...
                    <div className={`pt-12 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} space-y-8 transition-colors duration-700`}>
                      ...
                      <div className="flex flex-wrap gap-4">
                          {[{ id: 'sm', label: 'ছোট' }, { id: 'base', label: 'মাঝারি' }, { id: 'lg', label: 'বড়' }, { id: 'xl', label: 'অতিরিক্ত বড়' }].map(size => (
                            <button key={size.id} onClick={() => handleFontSizeChange(size.id)} className={`px-10 py-4 rounded-2xl border-2 transition-all duration-700 font-black bn-serif text-lg ${fontSize === size.id ? 'bg-amber-500 text-white border-amber-500 shadow-xl' : 'bg-white/5 border-white/5 text-slate-500 hover:bg-white/10'}`}>{size.label}</button>
                          ))}
                      </div>
                    </div>
                    ...
                    <div className={`pt-12 border-t ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} space-y-8 transition-colors duration-700`}>
                      ...
                      <a 
                        ...
                        className={`flex items-center justify-between p-8 rounded-[2rem] border-2 transition-all duration-700 group ${theme === 'dark' ? 'bg-white/5 border-white/5 hover:border-amber-500/30' : 'bg-black/5 border-black/5 hover:border-amber-500/30'}`}
                      >
                         <div className="flex items-center gap-6">
                           ...
                           <div className="space-y-1">
                             <p className={`font-black bn-serif transition-colors duration-700 ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>ফিডব্যাক বা সমস্যা রিপোর্ট করুন</p>
                ...
                <div className={`relative group overflow-hidden divine-glass p-12 md:p-16 rounded-[4.5rem] shadow-3xl border-2 transition-all duration-700 ${theme === 'dark' ? 'border-white/5 hover:border-amber-500/20' : 'border-black/5 hover:border-amber-500/30'}`}>
                   ...
                   <div className="flex flex-col md:flex-row gap-12 items-center md:items-start relative z-10">
                      <div className="relative">
                        <div className={`w-44 h-44 ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'} rounded-[3.5rem] flex items-center justify-center border-4 border-amber-500/10 shadow-2xl overflow-hidden group-hover:scale-105 transition-all duration-700 ring-4 ring-amber-500/30`}>
                          <img 
                      ...
                      <div className="space-y-8 text-center md:text-left flex-1">
                         <div className="space-y-3">
                           <h3 className={`text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'} bn-serif tracking-tight transition-colors duration-700`}>পবিত্র বানী <span className="text-amber-500">ডেভলপার টিম</span></h3>
                           <p className="text-amber-600 font-black text-xs uppercase tracking-[0.5em]">শান্তি ও প্রজ্ঞার কারিগর</p>
                         </div>
                         
                         <p className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-700'} leading-relaxed bn-serif text-lg font-medium text-justify max-w-2xl transition-colors duration-700`}>
                         ...
                         <div className="flex flex-wrap justify-center md:justify-start gap-4">
                            <a 
                              ...
                              className="flex items-center gap-4 px-8 py-5 bg-amber-600 hover:bg-amber-500 text-white rounded-[2rem] shadow-xl hover:shadow-amber-500/30 active:scale-95 transition-all duration-700 group/call"
                            >
      ...
      <nav className="fixed bottom-8 left-8 right-8 md:hidden z-50">
        <div className="divine-glass flex justify-around p-5 rounded-[2.5rem] shadow-3xl border-white/10 backdrop-blur-3xl transition-all duration-700">
...
const RecommendItem: React.FC<{ title: string; desc: string; icon: string; onClick: () => void; theme: string }> = ({ title, desc, icon, onClick, theme }) => (
  <button onClick={onClick} className="divine-card p-12 rounded-[3.5rem] text-left group flex items-center justify-between transition-all duration-700 hover:scale-[1.03] hover:shadow-2xl hover:border-amber-500/30">
    <div className="flex gap-10 items-center">
       <div className={`w-20 h-20 ${theme === 'dark' ? 'bg-white/5' : 'bg-black/5'} rounded-[2rem] flex items-center justify-center text-slate-500 group-hover:bg-amber-500/15 group-hover:text-amber-500 transition-all duration-700 shadow-inner border border-white/5`}>
         <i className={`fa-solid ${icon} text-3xl`}></i>
       </div>
       <div className="space-y-2">
         <p className="text-[11px] font-black uppercase tracking-[0.5em] text-amber-600/50">{desc}</p>
         <h4 className={`text-2xl font-bold ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'} bn-serif group-hover:text-amber-600 transition-colors duration-700`}>{title}</h4>
       </div>
    </div>
    <div className={`w-14 h-14 rounded-full border-2 ${theme === 'dark' ? 'border-white/5' : 'border-black/5'} flex items-center justify-center text-slate-500 group-hover:text-amber-500 group-hover:border-amber-500/30 transition-all duration-700`}>
...
const MobileNavItem: React.FC<{ icon: string; active: boolean; onClick: () => void }> = ({ icon, active, onClick }) => (
  <button onClick={onClick} className={`w-16 h-16 flex items-center justify-center rounded-2xl transition-all duration-700 ${active ? 'bg-amber-500 text-white shadow-xl scale-110' : 'text-slate-500 hover:text-slate-300'}`}>
...
const SocialIcon: React.FC<{ icon: string; link: string }> = ({ icon, link }) => {
  ...
  return (
    <a 
      ...
      className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:text-amber-500 hover:bg-amber-500/10 transition-all duration-700 border-2 border-white/5 hover:border-amber-500/30 shadow-sm active:scale-90"
    >
